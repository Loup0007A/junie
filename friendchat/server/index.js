...
app.get('/api/admin/users', adminAuth, async (req, res) => {
  try {
    ...
    res.json(users.map(u => ({ id: u.id, username: u.username, email: u.email || null, role: u.role, status: u.status, avatar: u.avatar || null, muted: u.muted || false, mute_until: u.muteUntil || null, banned: u.banned || false, invisible: u.invisible || false, mustChangePassword: u.mustChangePassword || false, created_at: u.createdAt, last_seen: u.lastSeen || null })));
  } catch { res.status(500).json({ error: 'Server error' }); }
});
...
// Routed through the backend so API keys stay server-side and
// browser CORS/network restrictions never affect the client.
const GIPHY_KEY = process.env.GIPHY_KEY || 'dc6zaTOxFJmzC'; // public Giphy demo key
const TENOR_KEY = process.env.TENOR_KEY || 'LIVDSRZULELA';   // public Tenor demo key
...
app.get('/api/gifs/search', auth, async (req, res) => {
  try {
    ...
    let results = [];
    if (source === 'tenor') {
      const url = `https://tenor.googleapis.com/v2/search?key=${TENOR_KEY}&client_key=friendchat&q=${encodeURIComponent(q)}&limit=${limit}&media_filter=gif&contentfilter=medium`;
      const r = await fetch(url);
      if (!r.ok) throw new Error(`Tenor ${r.status}`);
      const d = await r.json();
      results = (d.results || []).map(g => ({
        id: g.id,
      ...
      const url = `https://api.giphy.com/v1/gifs/search?api_key=${GIPHY_KEY}&q=${encodeURIComponent(q)}&limit=${limit}&rating=pg&lang=fr`;
      const r = await fetch(url);
    ...
    res.status(502).json({ error: 'GIF provider unavailable', results: [] });
  }
});
...
app.get('/api/gifs/trending', auth, async (req, res) => {
  try {
    ...
    let results = [];
    if (source === 'tenor') {
      const url = `https://tenor.googleapis.com/v2/featured?key=${TENOR_KEY}&client_key=friendchat&limit=${limit}&media_filter=gif&contentfilter=medium`;
      const r = await fetch(url);
      if (!r.ok) throw new Error(`Tenor ${r.status}`);
      const d = await r.json();
      results = (d.results || []).map(g => ({
      ...
      const url = `https://api.giphy.com/v1/gifs/trending?api_key=${GIPHY_KEY}&limit=${limit}&rating=pg`;
      const r = await fetch(url);
      if (!r.ok) throw new Error(`Giphy ${r.status}`);
      const d = await r.json();
      results = (d.data || []).map(g => ({
