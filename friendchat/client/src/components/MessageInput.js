...
const GIPHY_KEY = process.env.REACT_APP_GIPHY_KEY || 'dc6zaTOxFJmzC'; // public test key
const TENOR_KEY = process.env.REACT_APP_TENOR_KEY || 'AIzaSyAyimkuYQYF_FXVALexPMidhINAGwJn8Ik'; // demo key
...
export default function MessageInput({ onSend, onTyping, disabled, placeholder = 'Écrire un message…' }) {
  ...
  const searchGifs = async (query) => {
    ...
    try {
      let results = [];
      if (gifSource === 'giphy') {
        const r = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${GIPHY_KEY}&q=${encodeURIComponent(query)}&limit=15&rating=pg`);
        const d = await r.json();
        results = d.data?.map(g => ({ id: g.id, url: g.images.fixed_height_small.url, preview: g.images.fixed_height_small.url })) || [];
      } else {
        const r = await fetch(`https://tenor.googleapis.com/v2/search?key=${TENOR_KEY}&q=${encodeURIComponent(query)}&limit=15&media_filter=gif`);
        const d = await r.json();
        results = d.results?.map(g => ({ id: g.id, url: g.media_formats.gif.url, preview: g.media_formats.tinygif?.url || g.media_formats.gif.url })) || [];
      }
      setGifs(results);
    } catch (err) {
      ...
      (async () => {
        ...
        try {
          ...
          if (gifSource === 'giphy') {
            ...
            const r = await fetch(`https://tenor.googleapis.com/v2/featured?key=${TENOR_KEY}&limit=12&media_filter=gif`);
            const d = await r.json();
            results = d.results?.map(g => ({ id: g.id, url: g.media_formats.gif.url, preview: g.media_formats.tinygif?.url || g.media_formats.gif.url })) || [];
          }
          setGifs(results);
            ...
            <div style={{
              ...
              <div style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                  <button className={`btn btn-sm ${gifSource === 'giphy' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => { setGifSource('giphy'); setGifs([]); setGifSearch(''); }}>
                    Giphy
                  </button>
                  <button className={`btn btn-sm ${gifSource === 'tenor' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => { setGifSource('tenor'); setGifs([]); setGifSearch(''); }}>
                    Tenor
                  </button>
                </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: 8, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4 }}>
                {gifLoading ? (
                  ...
                  <img
                    ...
                    src={gif.preview}
                    alt="gif"
                    style={{ width: '100%', height: 80, objectFit: 'cover', borderRadius: 6, cursor: 'pointer', transition: 'opacity 0.15s' }}
                    onClick={() => sendGif(gif)}
