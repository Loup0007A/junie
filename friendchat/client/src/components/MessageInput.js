...
export default function MessageInput({ onSend, onTyping, disabled, placeholder = 'Écrire un message…' }) {
  ...
  const searchGifs = async (query) => {
    ...
    try {
      ...
      console.error('GIF search error:', msg);
      setGifError(msg);
      setGifs([]);
    } finally {
      setGifLoading(false);
    ...
    if (showGif && gifs.length === 0 && !gifSearch) {
      (async () => {
        ...
        try {
          ...
          const msg = err.response?.data?.error || err.message || 'Erreur inconnue';
          console.error('GIF trending error:', msg);
          setGifError(msg);
        } finally {
          setGifLoading(false);
        }
            ...
            <div style={{
              ...
              <div style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                  <button className={`btn btn-sm ${gifSource === 'giphy' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => { setGifSource('giphy'); setGifs([]); setGifSearch(''); setGifError(null); }}>
                    Giphy
                  </button>
                  <button className={`btn btn-sm ${gifSource === 'tenor' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => { setGifSource('tenor'); setGifs([]); setGifSearch(''); setGifError(null); }}>
                    Tenor
                  </button>
                </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: 8, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, minHeight: 120 }}>
                {gifLoading ? (
                  ...
                  <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '16px 10px', fontSize: 12, lineHeight: 1.6 }}>
                    <div style={{ fontSize: 22, marginBottom: 6 }}>😕</div>
                    <div style={{ color: 'var(--danger)', fontWeight: 600, marginBottom: 4 }}>GIF indisponible</div>
                    <div style={{ color: 'var(--text-muted)', fontSize: 11, marginBottom: 10 }}>
                      {gifError.includes('Clé API') ? (
                        <>Ajoutez <code style={{ color: 'var(--accent-light)' }}>GIPHY_KEY</code> / <code style={{ color: 'var(--accent-light)' }}>TENOR_KEY</code><br/>dans vos variables d'environnement Render.</>
                      ) : gifError}
                    </div>
                    <button className="btn btn-secondary btn-sm" onClick={() => { setGifError(null); setGifs([]); if (gifSearch) searchGifs(gifSearch); }}>
                      Réessayer
                    </button>
                  </div>
                ) : gifs.length === 0 ? (
